...
| Resolve profile | `/development-swift:resolve-profile` (not invoked by hand — loaded by name by `/development:resolve-issue` at its §1b step) | The swift repo type's driver rules for the resolve-issue conductor — six contract headings (named once in the *Resolve profile contract* in ARCHITECTURE.md, not restated here) holding the blessed whole-suite gate for both build systems (`swift build && swift test --enable-code-coverage`, or `xcodebuild` for an Xcode project, pinned against `swift-ci-fixer`), `--gate-attest: not applicable`, a *conditional* `none` version bump (an app repo ordinarily ships no installable plugin content — but a detected language beats the `claude-plugin` fallback, so a repo that is both loads *this* profile and §4's floor still applies to its `<plugin>/` tree), and the panel pointer. Not for direct use; see the *Resolve profile contract* in [`ARCHITECTURE.md`](https://github.com/timo-jakob/timos-claude-code-plugins/blob/main/ARCHITECTURE.md). |
...
| Resolve profile | `/development-python:resolve-profile` (not invoked by hand — loaded by name by `/development:resolve-issue` at its §1b step) | The python repo type's driver rules for the resolve-issue conductor — six contract headings (named once in the *Resolve profile contract* in ARCHITECTURE.md, not restated here) holding the blessed whole-suite gate (`pytest --cov …`, run through the repo's own virtualenv interpreter, pinned against `python-ci-fixer`), `--gate-attest: not applicable`, a *conditional* `none` version bump (an app repo ordinarily ships no installable plugin content — but a detected language beats the `claude-plugin` fallback, so a repo that is both loads *this* profile and §4's floor still applies to its `<plugin>/` tree), and the panel pointer. Not for direct use; see the *Resolve profile contract* in [`ARCHITECTURE.md`](https://github.com/timo-jakob/timos-claude-code-plugins/blob/main/ARCHITECTURE.md). |
...
| Resolve profile | `/development-java:resolve-profile` (not invoked by hand — loaded by name by `/development:resolve-issue` at its §1b step) | The java repo type's driver rules for the resolve-issue conductor — six contract headings (named once in the *Resolve profile contract* in ARCHITECTURE.md, not restated here) holding the blessed whole-suite gate (`./gradlew build test jacocoTestReport`, pinned against `java-ci-fixer`; Gradle-only, so there is no `mvn` arm), `--gate-attest: not applicable`, a *conditional* `none` version bump (an app repo ordinarily ships no installable plugin content — but a detected language beats the `claude-plugin` fallback, so a repo that is both loads *this* profile and §4's floor still applies to its `<plugin>/` tree), and the panel pointer. Not for direct use; see the *Resolve profile contract* in [`ARCHITECTURE.md`](https://github.com/timo-jakob/timos-claude-code-plugins/blob/main/ARCHITECTURE.md). |
...
| Resolve profile | `/development-kubernetes:resolve-profile` (not invoked by hand — loaded by name by `/development:resolve-issue` at its §1b step) | The kubernetes repo type's driver rules for the resolve-issue conductor — six contract headings (named once in the *Resolve profile contract* in ARCHITECTURE.md, not restated here) holding the blessed gate, which is validation rather than a suite: render first, then `kubeconform` + `kube-linter` + `kyverno test`, anchored on this plugin's **maintenance** skill because it ships no ci-fixer agent (a manifest repo has no CI build to repair), with an anti-no-op rule over the panel's **tree** (rendered output *plus* the standalone manifests and the declared policy tree, after the panel's documented skips) rather than over the render set, reached only once the diff carried a deploy-relevant path — a repo-wide empty tree is reported and stopped on, while a non-empty tree none of whose documents belongs to the change is carried to §3.5's not-applicable arm rather than halted here; plus a tool the repo's own markers select but that is not installed reported as unverified rather than green (a tool those markers never select is not missing at all); plus `--gate-attest: not applicable`, a `none` version bump (a cluster-definition repo ships no installable plugin content), and the panel pointer. Not for direct use; see the *Resolve profile contract* in [`ARCHITECTURE.md`](https://github.com/timo-jakob/timos-claude-code-plugins/blob/main/ARCHITECTURE.md). |
...
| Resolve profile | `/development-go:resolve-profile` (not invoked by hand — loaded by name by `/development:resolve-issue` at its §1b step) | The go repo type's driver rules for the resolve-issue conductor — six contract headings (named once in the *Resolve profile contract* in ARCHITECTURE.md, not restated here) holding the blessed whole-suite gate (`go build ./... && go test ./...`, pinned against `go-ci-fixer`, with its judge-by-exit-status rule and its "a `-race` failure is a real defect" rule), `--gate-attest: not applicable`, a *conditional* `none` version bump (an app repo ordinarily ships no installable plugin content — but a detected language beats the `claude-plugin` fallback, so a repo that is both loads *this* profile and §4's floor still applies to its `<plugin>/` tree), and the panel pointer. Not for direct use; see the *Resolve profile contract* in [`ARCHITECTURE.md`](https://github.com/timo-jakob/timos-claude-code-plugins/blob/main/ARCHITECTURE.md). |
