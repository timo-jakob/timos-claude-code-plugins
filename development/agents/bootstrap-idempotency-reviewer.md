...
- The current file is a placeholder generated by GitHub's UI (e.g.,
  ...
  edited.
- **The current file is a RETIRED TEMPLATE the new one supersedes** — the
  on-disk content is a previous version of this same shipped artifact, not
  the user's work, and the two are not mergeable by design. The live case is
  `.spectral.yaml` (#689): recognise the retired #692 starter by shape —
  `extends: ["spectral:oas"]`, its own `rules:` block, and no `cdn.jsdelivr.net`
  pin — against a template that is a single pinned `extends`. Recommend
  `overwrite`. This is the one shape where `merge` is the harmful answer:
