...
# model (haiku|sonnet|opus|fable) + tools, and `name` must match the filename.
...
if (( ${#md_targets} )); then
  ...
  for file in "${md_targets[@]}"; do
    ...
    if [[ "$kind" == "agent" ]]; then
      ...
      case "$model_val" in
        ""|haiku|sonnet|opus|fable) ;;   # empty already reported as missing_field
        *)
          add_skill_finding "$(jq -n --arg file "$file" --arg got "$model_val" '{
            ...
            message: ("agent \($file): model “\($got)” is not one of haiku|sonnet|opus|fable"),
            fix: "set model to haiku, sonnet, opus, or fable",
            files: [$file] }')"
          ;;
      esac
    fi
