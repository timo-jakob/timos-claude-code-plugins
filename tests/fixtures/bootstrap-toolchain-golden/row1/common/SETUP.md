...
## 3h. No direct-to-cluster deploys (`no-cluster-deploy`) — application repositories only

> **Infrastructure (GitOps) repositories: this section does not apply.** If
> `.maintenance.yml` records `primary: kubernetes`, bootstrap installs neither
> half of this gate and `branch-protection.sh --iac-only true` never requires
> the context — your required check is `kubernetes-ci.yml`'s single `gate`
> instead. Writing to a cluster is what an infrastructure repository is *for*.

**Nothing for you to do — this is what the check means when it fires.**

This repository is an **application repository**, and the family's promotion
contract says an application repository never writes to a cluster itself. It
publishes versioned, immutable images; a version change reaches a cluster as a
**pull request against the infrastructure repository**. That single rule is what
makes every cluster change reviewable and auditable after the fact — see
[issue #1189](https://github.com/timo-jakob/timos-claude-code-plugins/issues/1189)
for the position and its rationale.
...
**The exact v1 command set** — closed, and stated in the checker's own header:

| Fails the check | Passes the check |
| --- | --- |
...
**Three exemptions — two scoped, one repo-wide:**

- **An ephemeral cluster the job stands up itself** (job-scoped). A job that
  runs `kind create cluster`, `k3d cluster create`, `minikube start` or `ctlptl
  apply` exempts the cluster-writing steps *at or after* it in that same job —
  including the rest of the creating step's own body, so a single `run: |` that
  stands up kind and then applies to it is exempt. An integration test that
  spins up kind and applies manifests to it is not a deploy. The exemption keys on
  the **earliest** creating step in the job and on its condition: an
  unconditional one exempts every cluster-writing step at or after it, while one
  carrying an **`if:`
  guard exempts only a step carrying the byte-identical condition**, because the
  branch-per-environment shape (`if: pull_request` stands up kind, `if: ref ==
  main` deploys) is exactly what the gate exists to catch — there the two are
  mutually exclusive and the deploy reaches the real cluster, so a differing
  condition, or an unguarded deploy under a guarded creator, fails closed. Both
  steps guarded alike is the commonest real integration shape and keeps the
  exemption. The exemption is **job-scoped**: the same step in a sibling job still
  fails, because a sibling job writes to whatever cluster its own credentials
  point at.
- **A command carrying a real dry-run flag**, **command-scoped**: bare
  `--dry-run`, `--dry-run=client`, `--dry-run=server`, `--dry-run=true`, or the
  separate-argument `--dry-run client|server`. **`--dry-run=none` and
  `--dry-run=false` do not exempt** — `none` is kubectl's default and both mean
  "really do it". Note the scope: it exempts *that command*, not the whole step,
  so a step that dry-runs and then really applies still fails on the second
  command. A step-wide exemption would be a one-line switch for silencing the
  check, which is the escape hatch the section below says does not exist.
- **A repository recording `primary: kubernetes`** (repo-wide) — see
  *Infrastructure repositories* below.

**There is deliberately no escape hatch** — no `# allow` annotation, no
allowlist file. The whole value of this rule is auditability, and an annotation
that switches it off in the file that breaks it is exactly the audit trail the
rule exists to protect. A repository with a genuine exception **changes the
check**, in a reviewed pull request a human reads. Outgrowing a decision earns a
deliberate re-decision, which is not the same as a standing escape hatch.

**Infrastructure repositories are exempt as a whole.** The checker exits 0 when
`.maintenance.yml` records `primary: kubernetes` — an infrastructure repository
is the one place a cluster write belongs, and its own required check is
`kubernetes-ci.yml`'s `gate` job instead. A repository that carries Kubernetes
manifests **and** records a language primary is an application repository, and
is checked.

**Known gaps (v1).** A pass means "no cluster-writing command appears literally
in a workflow's own `run:` text" — it is not proof the repository never writes to
a cluster:

- Only `run:` script bodies are inspected. A deploy expressed as a marketplace
  action (`uses: azure/k8s-deploy@v4` and friends) or as a reusable workflow is
  not detected yet.
- **Indirection is not followed** — `run: ./scripts/deploy.sh`, `run: make
  deploy`, or a composite action under `.github/actions/**` reaches a cluster
  with nothing here to see.
- The ephemeral exemption reads `run:` bodies for the same reason, so a job that
  stands its cluster up with `helm/kind-action` is **not** exempt and goes red.
  That direction fails *closed* — a false alarm you can see, rather than a
  silent pass. Your recourse is to stand the cluster up in a `run:` step
  (`kind create cluster`), or to change the check in a reviewed pull request;
  there is no annotation that will silence it.
...
- An `if:` written as a YAML **boolean** is not a string, so the checker records
  no condition and reads that creator as unconditional. The two spellings differ:
  `if: true` always runs, so "unconditional" is the **correct** reading; but
  `if: false` — the usual way to park a step — never runs and creates no
  cluster, and yet still exempts every cluster-writing step at or after it,
  unguarded ones included. That direction fails **open**: a `kind create
  cluster` parked with `if: false` currently exempts a real `helm upgrade
  --install` after it. Write the guard as an expression string
...
- Some cluster-mutating verbs are outside the closed v1 set **by decision, not
  ...
  install`. The rule still forbids them; the check simply does not catch them
  yet. Widening the set is a deliberate re-decision in a reviewed pull request.
- A trailing `# …` comment is dropped from the command it follows, so prose
  after a real command neither triggers a match nor silences one.
...
> ⚠️ **`SETUP.md` is a scaffold file, so it is deliberately not drift-tracked.**
> A repository that adopts this check later — through `/development:bootstrap`'s
> gap-fill, which delivers `scripts/check-no-cluster-deploy.zsh` and
> `.github/workflows/no-cluster-deploy.yml` — will **not** receive this section
> automatically, because bootstrap never overwrites a customized scaffold file.
> Copy it across by hand, or read the checker's header, which carries the same
> rules.
...
needed. The `permissions: packages: write` block on the `push-and-sign` job
grants the required scope (only that job holds it; PR runs never execute it).

### 5.2 Image visibility — one-time manual step

GHCR creates new container packages as **private** by default, and GitHub
provides **no API or CLI to change package visibility** — it is web-UI-only, so
this step is genuinely manual (there is nothing to automate). After your first
publish:

- **Public repos that want a public image** — open the package settings:
  - user-owned repo:
  ...
  Scroll to "Danger Zone" → "Change visibility" → Public. **⚠️ Making a package
  public is irreversible** — GitHub does not allow changing it back to private.
  If this repo also publishes a docs image (the `<repo>-docs` package), repeat
  the flip for that package at the same URL pattern.
...
the `push-and-sign` job in your `quality-*.yml` workflow (`quality-public.yml`
for public repos, `quality-private.yml` for private):
...
2. Add a Docker Hub image to `docker/metadata-action`'s `images` input
   (newline-separated) — the `Compute image tags` step appears in both the
   `image` (scan) job and `push-and-sign`; update both so the scanned tags
   stay aligned with the published ones:
   ```yaml
   images: |
