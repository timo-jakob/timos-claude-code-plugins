...
| `prune-stub.bash` | Shared `grep` stub for the #1428 prune-filter tests (`load prune-stub`) — `failing_prune_grep_stub DIR` fails only the invocation carrying the kubernetes prune operand (`/templates/`), delegates every other grep, proves it fired via `DIR/fired` (every caller asserts it), and writes to stderr so a tolerant test's `[ -z "$stderr" ]` is falsifiable. Build it BEFORE putting `DIR` on `PATH`: it refuses (non-zero, cause on stderr) when no executable `grep` is on `PATH` or when the one found already lives under `DIR` |
| `prune-stub.bats` | Unit coverage for the above — delegation, the `/templates/` intercept (exit 2, stderr line, `fired`), the two refusals, the rebuild clearing `fired`, and a `DIR` containing a space (pins the `%q` interpolation) |
