...
| `marker-find-stub.bash` | Shared `find` stub for the kubernetes marker's find-half tests (`load marker-find-stub`, #1177 reshaped by #1393) — `failing_marker_find_stub DIR` fails only the invocation carrying the marker operand `kustomization.yml` (not `Chart.yaml`, which detect-stack's `detection_confidence` probe also finds), running the real find first so its output survives (a walk that died after printing), delegates every other find, proves it fired via `DIR/fired` (every caller asserts it), and writes to stderr so a tolerant test's `[ -z "$stderr" ]` is falsifiable. It exists because since #1393 both halves of the marker skip the same trees, so no directory can fail find alone. Build it BEFORE putting `DIR` on `PATH`: it refuses (non-zero, cause on stderr) with no `DIR`, when no executable `find` is on `PATH`, or when the one found already lives under `DIR` |
...
| `philosophy-links.bats` | Propagation invariant for the **philosophy page's links** (#1631) — every `blob/main/` / `tree/main/` repo link on a docs page resolves via `git ls-files`, and every shipped-plugin `#pillar-N` citation resolves to one `{#pillar-N}` id on `docs/explanation/philosophy.md`; derived rosters, roster tripwire against `MAINTAINING.md` (whose row names `tests/philosophy-links.bats`), docs-page + `SKILL.md` non-vacuity controls |
| `reviewer-evidence-rule.bats` | Propagation invariant for the **#1584 evidence rule** — every read-only reviewer repo-wide (normalised frontmatter tool set `Read, Grep, Glob`, derived from `git ls-files '*/agents/*.md'`, #1644) carries the rule under one heading, byte-identically, with a `decides:` / `proposed-severity:` slot in its Reporting Format — unless `reviewer-evidence-rule.exemptions` names it with a reason, which is gated both ways; also pins the conductor's decided pass in `reference/review-loop.md` and the two pointers at it. Derived roster, roster tripwire against `MAINTAINING.md`, `needle_once` uniqueness control |
...
| `flake-hunt.bats` | Tests `scripts/flake-hunt.zsh`, the load-dependent flake hunt (#1796), with `bats` and `parallel` stubbed. It covers the report format, numeric ordering and file attribution, and the forced contention: `--jobs`, `LC_ALL=C`, and one running busy loop per CPU before the first iteration. It covers the refusal of a broken iteration (zero tests, fewer than planned, a failing exit with no failure) and the usage taxonomy. It also checks that nothing the hunt started is left after a normal exit, an INT, a TERM, a HUP, a closed output pipe, or when a Ctrl-C lands during cleanup. That includes a child the bats run put in a group of its own, as GNU parallel does, and one that ignores TERM. A group whose number now belongs to another process is never signalled. When to run the hunt itself: MAINTAINING.md, *Hunting load-dependent flakes* |
