...
| `reviewer-evidence-rule.bats` | Propagation invariant for the **#1584 evidence rule** — every read-only reviewer (normalised frontmatter tool set `Read, Grep, Glob`) carries the rule under one heading, byte-identically, with a `decides:` / `proposed-severity:` slot in its Reporting Format; also pins the conductor's decided pass in `reference/review-loop.md` and the two pointers at it. Derived roster, roster tripwire against `MAINTAINING.md`, `needle_once` uniqueness control |
