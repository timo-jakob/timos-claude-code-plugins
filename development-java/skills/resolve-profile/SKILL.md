...
- **A missing JaCoCo plugin is not a red gate.** Gradle then fails with *Task
  ...
  **Carry that forward to §6, because §6 will not work it out for itself.**
  ...
  absence*; it never checks whether the build could produce one, so it will ask
  for a report this project cannot generate. Do **not** satisfy it by applying
  JaCoCo — that is the separate change above, now smuggled in — and do **not**
  push with `--no-verify`, which walks past the coverage floor. Report the
  missing coverage tooling and stop.
