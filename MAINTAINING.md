...
whatever the table above would say. **Membership is a condition, not a list**:
any plugin whose `tests/<topic>-plugin-skeleton.bats` asserts a version prefix
is under it, because that assertion is what reds when the label and the manifest
disagree. Today three are — `development-kubernetes` (`0.3.`),
`development-opentofu` (`0.3.`) and `development-composition` (`0.1.`) — each
with its label reproduced in `README.md` and `docs/reference/plugins.md`. So a
...
{ grep -E '"[a-z-]+"' development/skills/bootstrap/scripts/preflight.sh | grep -v "^\s*#"; sed -n 's/^iac_brews=(\(.*\))$/\1/p' development/skills/bootstrap/scripts/preflight.sh | tr ' ' '\n'; } | sort -u

# CLI tool versions a workflow template downloads itself (the IaC kubernetes-ci's
# one `gate` job installs every tool its gate script runs by release URL, #1604 —
# these are NOT `uses:` pins, so Renovate never sees them and Step 2 never covers
# them). tests/iac-tools.zsh reads its fixture-harness pins from those same lines.
...
The IaC toolchain has **no pin outside that sweep** (#1604): `kubernetes-ci.yml.tmpl`'s
one `gate` job installs all seven tools its gate script runs — helm and kustomize
included, rather than taking the runner image's — and `tests/iac-tools.zsh` reads
the fixture harness's pins from those `install <tool>` steps. Bump a
`*_VERSION:` there and the harness moves with it.
...
| **IaC selection rule** — at most one IaC workflow per repo; the condition is the marker, not merely the absence of a language, with the explicit empty-GitOps-repo confirmation at bootstrap Q4 as the one exception (#1605) (the dual-marker halt is a #1162 specification, not current behaviour) (#1432) | `ARCHITECTURE.md` | `tests/iac-selection-rule.bats` | `grep -rlE 'iac[-_]only'` over `development docs ARCHITECTURE.md`, minus the authoritative site and `docs/superpowers/` — 7 roster files, 4 stating the selection, 8 selection statements |
...
| **#1584 evidence rule** — every read-only reviewer (frontmatter tool set `Read, Grep, Glob`) not named in `tests/reviewer-evidence-rule.exemptions` carries the rule under one heading, byte-identically, and gives `decides:` / `proposed-severity:` a slot in its Reporting Format; a claim about a tool RUN is capped at `SUGGESTION` while a defect read out of the artifacts is not. *(Summarised, not restated: the heading string and every operative clause live in the agents and in this sweep's own needle lists. Naming them here would add one more governed site.)* | `ARCHITECTURE.md` — *Terminating severity bars (#1433)*, the evidence-rule paragraph | `tests/reviewer-evidence-rule.bats`; the `ARCHITECTURE.md` statement itself is gated by `tests/claude-plugin-review-severity-bars.bats`, which reads the heading back out of the sweep rather than re-typing it | every tracked `*/agents/*.md` outside `tests/` (`git ls-files`, repo-wide since #1644) filtered on the NORMALISED read-only tool set (order- and spacing-insensitive), so roster == 37; the roster minus the exemption list is gated on every clause above, so gated == 34; each exemption must be a derived agent, state a reason and not carry the rule. The sweep reads both figures back OUT of this row and compares them to the derived counts, so a new read-only reviewer reds rather than shipping ruleless |
| **resolve-issue section pointers** — every `§N.M` pointer resolves to a `### N.M` heading in resolve-issue's `SKILL.md`, unless its own line names another document (`design doc`, `design spec`, `SETUP.md`), which makes it foreign and exempt; every step-2 spelling resolves to the round protocol's `2. **One loop invocation.**` item, found by that title, whose span still carries the not-applicable-on-a-full-round and `STALE_FINDINGS` arms; and the `### 3.5` section still routes to that protocol (#1492). Pointers to other steps index a different numbered list and are out of scope | `development/skills/resolve-issue/SKILL.md` — its `### N.M` headings; the step-2 item in `development/skills/resolve-issue/reference/review-loop.md` | `tests/reference-pointer-sweeps.bats` | `git ls-files '*.md' '*.zsh' '*.sh' '*.json'` minus `docs/superpowers/**` and `tests/**` — 91 resolve-issue pointers, 3 foreign pointers and 16 step-2 pointers (a step-2 pointer wrapped across two lines counts: each file is flattened first). The sweep reads all three figures back out of this row |
| **Work-dir artifact names** — every `<work-dir>/`-prefixed artifact the swept markdown names, normalised (prefix dropped, round token rewritten to `<N>`), is a stem the loop script writes, or a recorded session-written exception (#1492). Bare mentions without the prefix are out of scope | `development/skills/resolve-issue/scripts/resolve-story-loop.zsh` — every `$work_dir/…` stem it names | `tests/reference-pointer-sweeps.bats` | the `*.md` half of the section-pointer roster — 5 instruction-site files naming 15 distinct stems, one of them a session-written exception: `decided-<N>.log` (written in *The decided pass*). The sweep reads both figures and the exception list back out of this row |
| **Philosophy-page links** — every absolute repo link out of the docs resolves to a tracked path, and every shipped-plugin citation into the page resolves to a `{#pillar-N}` id; the red on a moved or renamed mechanism is the page's docs-impact signal (#1627, #1631). *(Pins resolution, not wording.)* | `docs/explanation/philosophy.md` | `tests/philosophy-links.bats` | `git ls-files docs \| grep -E '\.md$'` minus `docs/superpowers/**`, kept to pages carrying a `blob/main/` or `tree/main/` link — 22 pages, 98 links (96 `blob/main/` + 2 `tree/main/`), 51 distinct paths; and `git ls-files 'development*/**'` kept to files carrying `explanation/philosophy/#pillar-<digit>` — 4 files, 6 citations. The sweep reads all five figures back out of this row |
...
policy *kinds* the pinned CLI cannot evaluate — which the gate script that
workflow runs reports as a failure rather than silently passing, so a stale pin
surfaces as a red check in a consumer repo.
