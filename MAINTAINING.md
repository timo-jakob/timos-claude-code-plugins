...
| **#1584 evidence rule** — every read-only reviewer (frontmatter tool set `Read, Grep, Glob`) carries the rule under one heading, byte-identically, and gives `decides:` / `proposed-severity:` a slot in its Reporting Format; a claim about a tool RUN is capped at `SUGGESTION` while a defect read out of the artifacts is not. *(Summarised, not restated: the heading string and every operative clause live in the agents and in this sweep's own needle lists. Naming them here would add one more governed site.)* | `ARCHITECTURE.md` — *Terminating severity bars (#1433)*, the evidence-rule paragraph | `tests/reviewer-evidence-rule.bats`; the `ARCHITECTURE.md` statement itself is gated by `tests/claude-plugin-review-severity-bars.bats`, which reads the heading back out of the sweep rather than re-typing it | `development-claude-plugin/agents/*.md` filtered on the NORMALISED read-only tool set (order- and spacing-insensitive), gated on every clause above, so gated == roster == 5; the sweep reads that figure back OUT of this row and compares it to the derived count, so a sixth read-only reviewer reds rather than shipping ruleless. Extending the roster repo-wide is #1644 |
