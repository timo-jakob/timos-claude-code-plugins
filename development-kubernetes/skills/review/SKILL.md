...
wired in once, for every agent, so the reviewer definitions stay pure prose:
    ...
    Your evidence rule's "the tree you were told to read" is, for a `decides:` command, the source repository root above ({REPO}) — never the rendered temp tree, which does not exist where the command runs. Name source paths, and fold the render step (`helm template …` / `kustomize build …`) into the command when the verdict is about rendered output.
